...
class WtNet(nn.Module):
    ...
    def __init__(self, wavelet="db4", level=4, axis=2, in_chn=7, hid_chn=512, out_chn=7, activ="gelu", drop=0.0,
        ...
        self.feature_mlp = MLPBlock(1, self.out_chn, self.hid_chn, self.out_chn, self.activ, self.drop)
        # self.channel_mlp = MLPBlock(2, 11, self.hid_chn, self.in_chn, self.activ, self.drop)
        for i in range(self.level + 1):
            self.mlp_block_list.append(MLPBlock(1, self.feature[i], hid_chn, out_chn, activ, drop))
